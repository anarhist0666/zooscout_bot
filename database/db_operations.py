...
async def create_animal(
    ...
    physical_entity_id: int = None, 
    organization_id: int = None,
    # Новые параметры для фильтров
    city: str = None,
    gender: str = None,
    age_group: str = None,
    size: str = None
...
# Функция поиска животных с применением фильтров
async def get_animals_with_filters(
    category: str = None,
    city: str = None, 
    gender: str = None,
    age_group: str = None,
    ...
    async with async_session() as session:
        ...
        if city:
            query = query.where(Animal.city.ilike(f"%{city}%"))  # Поиск по частичному совпадению
        if gender:
            query = query.where(Animal.gender == gender)
        if age_group:
            query = query.where(Animal.age_group == age_group)
        if size:
        ...
        # Получаем дополнительную информацию о владельцах
        animals_data = []
        for animal in animals:
            # Получаем информацию о владельце
            user_result = await session.execute(
