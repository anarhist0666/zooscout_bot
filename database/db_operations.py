...
async def add_to_favorites(user_id: int, animal_id: int):
    async with async_session() as session:
        ...
        print(f"DEBUG add_to_favorites: user_id={user_id}, animal_id={animal_id}, existing={existing is not None}")
        ...
        print(f"DEBUG add_to_favorites: Successfully added favorite_id={favorite.id}")
        return True
...
async def is_animal_in_favorites(telegram_id: int, animal_id: int):
    async with async_session() as session:
        ...
        print(f"DEBUG is_animal_in_favorites: telegram_id={telegram_id}, user_id={user.id}, animal_id={animal_id}, found={favorite is not None}")
...
async def get_user_favorites_with_owners(user_id: int):
    async with async_session() as session:
        try:
            ...
            print(f"DEBUG: Found {len(results)} favorite-animal pairs")
            
            favorites_data = []
            
            for favorite, animal in results:
                print(f"DEBUG: Processing animal: {animal.name}, ID: {animal.id}")
                ...
                if not owner:
                    print(f"DEBUG: Owner not found for animal {animal.id}")
                    continue
            ...
            print(f"DEBUG: Successfully processed {len(favorites_data)} favorites")
            return favorites_data
            
        except Exception as e:
            print(f"ERROR in get_user_favorites_with_owners: {e}")
            return []
