...
from app.key import search_main, category_search, get_delete_keyboard, get_delete_physical_entity_keyboard, get_delete_organization_keyboard, get_attach_profile_keyboard, fill_form_menu, all_animals_viewed, get_like_keyboard, get_remove_favorite_keyboard, filter_category_kb, filter_city_kb, filter_gender_kb, filter_age_kb, filter_size_kb, start_search_with_filters_kb
...
# Новые состояния для расширенной фильтрации
...
async def process_photo(message: Message, state: FSMContext, bot: Bot):
    ...
    if total_profiles == 0:
        ...
        # Переходим к сбору данных для фильтров - город
...
# Обработчик для города животного
@router.message(AnimalForm.city)
async def process_animal_city(message: Message, state: FSMContext):
    if message.text == 'Отмена':
        await state.clear()
        await message.answer("❌ Создание анкеты отменено", reply_markup=kb.search)
        return
    ...
    await state.set_state(AnimalForm.gender)
    await message.answer(
        "🎯 Шаг 2/4: Выберите пол животного:",
...
# Обработчик для пола животного
@router.message(AnimalForm.gender, F.text.in_(['♂️ Мужской', '♀️ Женский']))
async def process_animal_gender(message: Message, state: FSMContext):
    if message.text == 'Отмена':
        await state.clear()
        await message.answer("❌ Создание анкеты отменено", reply_markup=kb.search)
        return
        
    gender_map = {'♂️ Мужской': 'Мужской', '♀️ Женский': 'Женский'}
    await state.update_data(gender=gender_map[message.text])
    await state.set_state(AnimalForm.age_group)
    await message.answer(
        "🎯 Шаг 3/4: Выберите возрастную группу:",
        reply_markup=ReplyKeyboardMarkup(
            keyboard=[
...
# Обработчик для возраста животного
@router.message(AnimalForm.age_group, F.text.in_(['🐣 До 5 лет', '🐕 Старше 5 лет']))
async def process_animal_age(message: Message, state: FSMContext):
    if message.text == 'Отмена':
        await state.clear()
        await message.answer("❌ Создание анкеты отменено", reply_markup=kb.search)
        return
        
    age_map = {'🐣 До 5 лет': 'До 5 лет', '🐕 Старше 5 лет': 'Старше 5 лет'}
    await state.update_data(age_group=age_map[message.text])
    await state.set_state(AnimalForm.size)
    await message.answer(
        "🎯 Шаг 4/4: Выберите размер животного:",
        reply_markup=ReplyKeyboardMarkup(
            keyboard=[
                [KeyboardButton(text='📏 Маленький'), KeyboardButton(text='📐 Средний')],
                [KeyboardButton(text='📏 Большой'), KeyboardButton(text='Отмена')]
            ],
            resize_keyboard=True,
            one_time_keyboard=True
        )
...
async def show_all_animals(message: Message, bot: Bot, state: FSMContext):
    ...
    if not animals_data:
        await message.answer(
            "😔 В базе пока нет анкет\n\n"
            "💡 *Станьте первым!*\n"
            "Создайте анкету своего питомца через раздел «Заполнение анкеты»",
...
# Обработчик для кнопки "Настроить фильтры"
@router.message(F.text == "⚙️ Настроить фильтры")
async def start_filter_setup(message: Message, state: FSMContext):
    # Сбрасываем состояние и начинаем настройку фильтров
    await state.clear()
    await state.set_state(FilterState.category)
    await message.answer(
        "🔧 Настройка фильтров\n\n"
        "Вы можете настроить параметры поиска или пропустить любой шаг.\n\n"
        "🎯 Шаг 1/5: Выберите категорию животного:",
        reply_markup=filter_category_kb
    )

# Обработчик выбора категории в фильтрах
@router.message(FilterState.category, F.text.in_(['🐶 Собаки', '🐱 Кошки', '🐾 Другие животные', '▶️ Пропустить']))
async def process_filter_category(message: Message, state: FSMContext):
    category_map = {
        '🐶 Собаки': 'Собака',
        '🐱 Кошки': 'Кошка', 
    ...
    # Переходим к следующему фильтру - город
    await state.set_state(FilterState.city)
    await message.answer(
        "🎯 Шаг 2/5: Укажите город:\n\n"
        "Нажмите '🏙️ Указать город' и введите название города, или пропустите этот шаг.",
        reply_markup=filter_city_kb
    )

# Обработчик для кнопки "Указать город"
@router.message(FilterState.city, F.text == '🏙️ Указать город')
async def request_city_input(message: Message, state: FSMContext):
    await message.answer(
        "🏙️ Введите название города:",
        reply_markup=ReplyKeyboardRemove()  # Убираем клавиатуру для ввода текста
    )

# Обработчик ввода города или пропуска
@router.message(FilterState.city)
async def process_filter_city(message: Message, state: FSMContext):
    if message.text == '▶️ Пропустить':
    ...
    # Переходим к следующему фильтру - пол
    await state.set_state(FilterState.gender)
    await message.answer(
        "🎯 Шаг 3/5: Выберите пол животного:",
...
# Обработчик выбора пола в фильтрах
@router.message(FilterState.gender, F.text.in_(['♂️ Мужской', '♀️ Женский', '▶️ Пропустить']))
async def process_filter_gender(message: Message, state: FSMContext):
    gender_map = {
        '♂️ Мужской': 'Мужской',
        '♀️ Женский': 'Женский'
    }
    ...
    # Переходим к следующему фильтру - возраст
    await state.set_state(FilterState.age_group)
    await message.answer(
        "🎯 Шаг 4/5: Выберите возрастную группу:",
        reply_markup=filter_age_kb
    )

# Обработчик выбора возраста в фильтрах
@router.message(FilterState.age_group, F.text.in_(['🐣 До 5 лет', '🐕 Старше 5 лет', '▶️ Пропустить']))
async def process_filter_age(message: Message, state: FSMContext):
    age_map = {
        '🐣 До 5 лет': 'До 5 лет',
        '🐕 Старше 5 лет': 'Старше 5 лет'
    }
    
    if message.text == '▶️ Пропустить':
        await state.update_data(age_group=None)
    else:
        await state.update_data(age_group=age_map[message.text])
    
    # Переходим к последнему фильтру - размер
    await state.set_state(FilterState.size)
    await message.answer(
        "🎯 Шаг 5/5: Выберите размер животного:",
        reply_markup=filter_size_kb
    )

# Обработчик выбора размера в фильтрах
@router.message(FilterState.size, F.text.in_(['📏 Маленький', '📐 Средний', '📏 Большой', '▶️ Пропустить']))
async def process_filter_size(message: Message, state: FSMContext):
    size_map = {
        '📏 Маленький': 'Маленький',
        '📐 Средний': 'Средний', 
        '📏 Большой': 'Большой'
    ...
    if message.text == '▶️ Пропустить':
        ...
        await state.update_data(size=size_map[message.text])
    
    # Все фильтры настроены, показываем сводку
    data = await state.get_data()
    
    # Формируем текст с выбранными фильтрами
    filters_text = "✅ Настроенные фильтры:\n"
    filters_text += f"• Категория: {data.get('category', '❌ Не выбрано')}\n"
    filters_text += f"• Город: {data.get('city', '❌ Не выбрано')}\n" 
    filters_text += f"• Пол: {data.get('gender', '❌ Не выбрано')}\n"
    filters_text += f"• Возраст: {data.get('age_group', '❌ Не выбрано')}\n"
    filters_text += f"• Размер: {data.get('size', '❌ Не выбрано')}\n\n"
    filters_text += "Нажмите '🔍 Найти по фильтрам' для поиска!"
    
    await message.answer(
        filters_text,
        reply_markup=start_search_with_filters_kb
    )
...
async def next_animal(message: Message, bot: Bot, state: FSMContext):
    # Проверяем, есть ли сохраненные данные поиска
    data = await state.get_data()
    animals_list = data.get('animals_list', [])
    current_index = data.get('current_index', 0)
    ...
    if current_index >= len(animals_list):
        await message.answer(
            ...
            "Больше анкет нет. Хотите начать поиск заново?",
            reply_markup=kb.all_animals_viewed
        )
        return
...
async def send_animal_profile(message: Message, animal_data: dict, bot: Bot, show_category: bool = False):
    ...
    try:
        ...
        if animal.gender:
            filter_info.append(f"♂️ Пол: {animal.gender}")
        if animal.age_group:
            filter_info.append(f"🐣 Возраст: {animal.age_group}")
        if animal.size:
...
# Обработчик для кнопки "Найти по фильтрам"
@router.message(F.text == '🔍 Найти по фильтрам')
async def start_filtered_search(message: Message, state: FSMContext, bot: Bot):
    # Получаем сохраненные фильтры из состояния
    data = await state.get_data()
    
    # Выполняем поиск с применением фильтров
    animals_data = await get_animals_with_filters(
    ...
    if not animals_data:
        # Формируем текст с использованными фильтрами
        used_filters = []
        if data.get('category'):
            used_filters.append(f"🐶 Категория: {data['category']}")
        if data.get('city'):
            used_filters.append(f"🏙️ Город: {data['city']}")
        if data.get('gender'):
            used_filters.append(f"♂️ Пол: {data['gender']}")
        if data.get('age_group'):
            used_filters.append(f"🐣 Возраст: {data['age_group']}")
        if data.get('size'):
            used_filters.append(f"📏 Размер: {data['size']}")
        
        filters_text = "\n".join(used_filters) if used_filters else "❌ Фильтры не применены"
        
        await message.answer(
            f"😔 По вашим фильтрам ничего не найдено\n\n"
            f"*Использованные фильтры:*\n{filters_text}\n\n"
            f"💡 *Советы:*\n"
            f"• Попробуйте ослабить фильтры\n"
            f"• Уберите некоторые параметры поиска\n"
            f"• Или посмотрите все анкеты подряд",
            reply_markup=kb.search_main,
            parse_mode='Markdown'
        )
        await state.clear()
    ...
    # Сохраняем результаты в состоянии для постраничного просмотра
    await state.update_data(
        current_index=0,
        animals_list=animals_data
    )
...
# Обработчик для кнопки "Назад к фильтрам"
@router.message(F.text == '↩️ Назад к фильтрам')
async def back_to_filters(message: Message, state: FSMContext):
    await state.set_state(FilterState.category)
    await message.answer(
...
async def process_animal_size(message: Message, state: FSMContext, bot: Bot):
    ...
    user_id = message.from_user.id
    if user_id not in animal_temp_data:
        await message.answer("❌ Ошибка: данные анкеты потеряны. Начните заново.", reply_markup=kb.search)
        await state.clear()
        return
    
    temp_data = animal_temp_data[user_id]
    
    # Создаем животное со всеми данными
    animal = await create_animal(
        user_id=temp_data['user_id'],
        category=temp_data['category'],
        name=temp_data['name'],
        description=temp_data['description'],
        photo_data=temp_data['photo_data'],
        telegram_file_id=temp_data['telegram_file_id'],
        physical_entity_id=temp_data.get('profile_id') if temp_data.get('profile_type') == 'physical' else None,
        organization_id=temp_data.get('profile_id') if temp_data.get('profile_type') == 'organization' else None,
    ...
    # Отправляем подтверждение
    await message.answer_photo(
        photo=temp_data['telegram_file_id'],
        caption=f"✅ Анкета питомца опубликована!\n\n"
                f"Теперь вашего питомца увидят пользователи в поиске. Хорошего дня!",
        reply_markup=kb.search
    )
